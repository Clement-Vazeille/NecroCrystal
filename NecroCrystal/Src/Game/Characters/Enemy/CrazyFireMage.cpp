...
void CrazyFireMage::SelectNewAction(sf::Vector2i& windowDimensions, float deltaTime, Map& map, std::vector<Character*>& characters, RandomLSFR& randomLSFR)
{
    ...
    if (newActionTimer >= newActionCooldown)
    {
        ...
        //Tourniquet: marche plutÙt lentement vers le nÈcromancien avec un tourniquet de feu autour de lui
        //  Plus probable si le necro est proche de lui
        //Dash: Dash (out ou in selon les hp actuels et la distance au necro)
        //  et laisse derriËre lui une trainÈe de feu imobile qui inflige des dÈgats
        //Explosion: Sort de base, au dÈbut de l'action met 3 vfx de prÈparation d'explosion sous le nÈcromancien
        //  puis ÅEla fin crÈer une explosion qui fait bien mal (l'explosion est un cercle qui s'agrandit)
        //Fury: un dash avec une vitesse augmentÈe, durÈe rÈduite,lance 1 explosions
        //  est lancÅEautomatiquement lorsqu'il est ÅE< 10%hp
